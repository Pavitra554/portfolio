import React from "react";
import Link from "next/link";

//React Icons
import { SiLeetcode,SiGithub,SiLinkedin,SiTwitter,SiInstagram} from "react-icons/si";

type props = {
  link: string;
  name: string;
};

const SocialButtonNormal: React.FC<props> = ({ link, name }) => {
  return (
    <Link href={link}>
        <div className='flex flex-row items-center p-2 rounded-md bg-neutral-800/60 text-neutral-400 shadow-md select-none hover:brightness-200 active:scale-90 ease-linear duration-100 cursor-pointer font-noto'>
          <div >
            { name === 'Leetcode' && <SiLeetcode size={25} /> }
            { name === 'Github' && <SiGithub size={25} /> }
            { name === 'LinkedIn' && <SiLinkedin size={25} /> }
            { name === 'Twitter' && <SiTwitter size={25} /> }
            { name === 'Instagram' && <SiInstagram size={25} /> }
          </div>
        </div>
    </Link>
  );
};

export default SocialButtonNormal;
